...
export default async function FaqsAdminPage() {
  await connectDB();
  const items = await Faq.find().sort({ displayOrder: 1 }).lean();
    ...
    <div className="space-y-6">
      ...
      <div className="grid gap-8 lg:grid-cols-[minmax(0,20rem)_1fr] lg:items-start">
          ...
          >
            <div>
              ...
              <Input
                name="question"
                required
                placeholder="How long to go live?"
              />
            </div>
            <div>
              <Label>Answer</Label>
              <Textarea
                ...
                required
                rows={4}
                placeholder="Clear answer for the public site…"
              />
            </div>
        ...
        <section>
          ...
          {items.length === 0 ? (
            <p className="text-sm text-stone-500">No FAQs yet.</p>
          ) : (
            <ul className="divide-y divide-stone-800 border-y border-stone-800">
              {items.map((f, index) => (
                <li key={String(f._id)} className="py-6">
                  <form
                    action={async (fd) => {
                      "use server";
                      await upsertFaq(String(f._id), fd);
                    }}
                    className="space-y-4"
                  >
                    <div className="flex flex-wrap items-center justify-between gap-3">
                      ...
                      <span
                        className={`text-xs font-semibold uppercase tracking-wider ${
                          f.active ? "text-emerald-400" : "text-stone-500"
                        }`}
                      >
